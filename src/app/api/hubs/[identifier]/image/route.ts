...
export async function POST(
  ...
  // Проверяем, является ли пользователь владельцем хаба
  ...
  try {
    ...
    const fileName = `${authUser.id}/${hubId}/${Date.now()}.${fileExt}`;
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("hub-images").getPublicUrl(fileName);
    ...
    if (hub?.image_url) {
      ...
      const urlParts = hub.image_url.split("/hub-images/");
      if (urlParts.length > 1) {
        const oldPath = urlParts[1].split("?")[0];
        if (oldPath && oldPath.startsWith(`${authUser.id}/${hubId}/`)) {
    ...
    // Обновляем хаб с новым URL изображения
    ...
    if (updateError) {
      ...
      await supabase.storage.from("hub-images").remove([fileName]);
      return NextResponse.json(
  ...
  try {
    ...
    if (hub?.image_url) {
      ...
      if (urlParts.length > 1) {
        ...
        if (path && path.startsWith(`${authUser.id}/${hubId}/`)) {
