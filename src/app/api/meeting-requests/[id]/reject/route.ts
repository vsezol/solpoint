...
export async function POST(
  ...
  try {
    ...
    if (meetingRequest.awaiting_user_id !== authUser.id) {
      return NextResponse.json({ error: "Only the awaiting user can reject this request" }, { status: 403 });
    }
    ...
    return meetingRequestsErrorResponse(error, "Failed to reject meeting request");
  }
}
