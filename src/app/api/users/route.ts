 ...
 * - current_user_id: required when best_matches=true
 * - limit: количество результатов (по умолчанию 200)
...
export async function GET(request: NextRequest) {
  ...
  if (mutualFriendsOnly && currentUserId) {
  ...
  const roles = parseCsv(searchParams.get("roles"));
  if (roles.length > 0) {
  ...
  const [
    ...
    bestMatches && currentUserId
      ...
      : Promise.resolve({ data: [], error: null }),
    bestMatches && currentUserId
      ? supabase.from("profile_skills").select("name").eq("user_id", currentUserId)
      : Promise.resolve({ data: [], error: null }),
    bestMatches
      ? supabase
          .from("event_members")
  ...
  const limit = Number.isNaN(limitRaw) || limitRaw < 1 ? 200 : Math.min(limitRaw, 1000);
