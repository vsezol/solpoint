...
ALTER TABLE public.hubs
  ADD CONSTRAINT IF NOT EXISTS check_hubs_country_code_uppercase 
...
ALTER TABLE public.hubs
  ADD CONSTRAINT IF NOT EXISTS check_hubs_country_code_length 
  CHECK (country_code IS NULL OR LENGTH(country_code) = 2);

-- Add foreign key to countries table for hubs
ALTER TABLE public.hubs
  ADD CONSTRAINT IF NOT EXISTS fk_hubs_country_code 
...
ALTER TABLE public.communities
  ADD CONSTRAINT IF NOT EXISTS check_communities_country_code_uppercase 
  CHECK (country_code IS NULL OR country_code = UPPER(country_code));

-- Add constraint to ensure country_code is 2 characters for communities
ALTER TABLE public.communities
  ADD CONSTRAINT IF NOT EXISTS check_communities_country_code_length 
  CHECK (country_code IS NULL OR LENGTH(country_code) = 2);

-- Add foreign key to countries table for communities
ALTER TABLE public.communities
  ADD CONSTRAINT IF NOT EXISTS fk_communities_country_code 
  FOREIGN KEY (country_code) REFERENCES public.countries(code)
  ON DELETE SET NULL;
...
ALTER TABLE public.projects
  ADD CONSTRAINT IF NOT EXISTS check_projects_country_code_uppercase 
  CHECK (country_code IS NULL OR country_code = UPPER(country_code));

-- Add constraint to ensure country_code is 2 characters for projects
ALTER TABLE public.projects
  ADD CONSTRAINT IF NOT EXISTS check_projects_country_code_length 
  CHECK (country_code IS NULL OR LENGTH(country_code) = 2);

-- Add foreign key to countries table for projects
ALTER TABLE public.projects
  ADD CONSTRAINT IF NOT EXISTS fk_projects_country_code 
  FOREIGN KEY (country_code) REFERENCES public.countries(code)
  ON DELETE SET NULL;
